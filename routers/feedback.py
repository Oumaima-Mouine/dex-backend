...
def get_feedback(db: Session = Depends(get_db)):
    result = db.execute(text("""
        SELECT id_reponse, matricule, sondage, repondu_le,
               note_globale, note_vitesse, note_stabilite,
               note_support, note_outils, commentaire, note_moyenne
        FROM feedback_etl
        ORDER BY repondu_le DESC
        LIMIT 50
    """))
...
def get_feedback_stats(db: Session = Depends(get_db)):
    result = db.execute(text("""
        SELECT
            ...
            ROUND(AVG(note_moyenne)::numeric, 2)   AS moy_generale,
